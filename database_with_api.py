...
def validate_user():
    username = request.form['p']
    password = request.form['u']
...
def get_items(category_id):
    items = db.execute("select  items.item_id,items.item_name from purchase.categories INNER JOIN purchase.items ON items.category_id=categories.id where category_id=\'{}\'".format(category_id))
...
def get_items_details(item_id , category_id):
    details = db.execute("select items.item_id,items.item_name,sellers.name as seller_name,items.price,items.description from purchase.items inner join purchase.sellers ON items.seller_id=sellers.id where item_id=\'{}\'".format(item_id))
